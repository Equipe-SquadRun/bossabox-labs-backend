...
describe('Testar a autenticação do sistema', () => {
    it('O POST com o endpoint /users/auth precisa retornar o status 200', async () => {
        ...
        await request(app).post('/usersTest').send(loginTeste)
        ...
        // Respostas esperadas
        const res = await await request(app).post('/usersTest/login').send(payload)
    ...
    it('O POST com o endpoint /users/logoff precisa retornar o status 200', async () => {        
        const res = await await request(app).post('/usersTest/logoff')
        expect(res.status).toEqual(200)        
    })
})